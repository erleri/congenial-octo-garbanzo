...
4. Add admin-only invoker RPC and history panel, timestamp+group cursor, no-change page advancement, legacy display-only groups. Clear history on auth changes, guard pending requests, and recheck active membership every 30 seconds while the page remains visible. Active-admin save/re-read, no-change filtering, pagination, legacy display, logout and timed external revocation passed on the isolated Preview project on 2026-09-11; production approval remains separate.
